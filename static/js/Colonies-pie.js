am4core.ready(function () {
   ...
   //chart.projection = new am4maps.projections.Mercator();
    ...
    // ===== map title =================
    var title = "Honey Production Distribution in 2019 by State"
    ...
    homeButton.events.on("hit", function () {
        ...
        zoomOut()
        morphBack
        countryLabel.text = "Select a State";
        countryLabel.show()
        am4core.ready
    });
    ...
    function morphBack() {
        if (morphedPolygon) {
            ...
            dsf.animate({ property: "saturation", to: 0.20 }, morphedPolygon.polygon.morpher.morphDuration);
        }
    }
    ...
    function morphToCircle(polygon) {
        ...
        polygon.defaultState.properties.fillOpacity = 0.7;
    ...
    function showPieChart(polygon) {
        ...
        var desaturated = fill.saturate(0.7);
...
//=============================== validate state clicked to show values in pie chart ========================
        ...
        console.log(data1)
        var state_ = data1.map(idstate => idstate.id)
        var category1 = data1.map(category1 => category1.yield_cny)
        var category2 = data1.map(category2 => category2.value)
        var category3 = data1.map(category3 => category3.prod_held_stocks)

        for (var t = 0; t < state_.length; t++) {
            if (polygon.dataItem._dataContext.id == state_[t]) {
                ...
                valores = [
                    ...
                    {
                       value: category3[t],
                        category: "prod_held_stocks"
                   }
                ]
                //console.log(valores)
                sum = category1[t] + category2[t] + category3[t]
            }
        } 
        //console.log(sum)
